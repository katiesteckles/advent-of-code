...
def in_da_hood(x,y,z,w):
    hood = set()
    for i in range(-1,2):
        for j in range(-1,2):
            for k in range(-1,2):
                for l in range(-1, 2):
                    hood.add((x+i, y+j, z+k, w+l))
    hood.remove((x,y,z,w))
    return hood
...
def iterate(points):
    ...
    for point in points:
        points_to_check |= in_da_hood(point[0],point[1],point[2],point[3])
    print('Checking ' + str(len(points_to_check)) + ' points...')
    points_to_add = set()
    points_to_remove = set()
    for check_point in points_to_check:
        da_hood = in_da_hood(check_point[0],check_point[1],check_point[2],check_point[3])
